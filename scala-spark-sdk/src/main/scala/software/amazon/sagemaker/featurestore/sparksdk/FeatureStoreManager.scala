...
class FeatureStoreManager(assumeRoleArn: String = null) extends Serializable {
   ...
   *
   * Only the supplied features are written; features that are not listed are preserved.
   * UpdateRecord is supported only for feature groups whose online store StorageType is
   * Standard_V2 or InMemory, and the record must already exist (use ingest_data / PutRecord
   * to create it). Pass the event-time feature as one of the feature name/value pairs.
   *
   * featureNames and featureValues are parallel lists (name[i] -> value[i]); they are passed
   * as two lists rather than a map so they marshal cleanly across the PySpark (py4j) bridge.
   *
   * @param featureGroupArn ARN or name of the Standard_V2 / InMemory feature group.
   * @param recordIdentifierValueAsString the record identifier value, in string format.
   * @param featureNames names of the features to update (max 100).
   * @param featureValues string values, positionally matched to featureNames.
   * @param targetStores optional stores to apply the update to; must include OnlineStore.
   * @param ttlDurationUnit optional TtlDuration unit (e.g. "Days"); requires ttlDurationValue.
   * @param ttlDurationValue optional TtlDuration value; requires ttlDurationUnit.
   */
  def updateRecord(
      featureGroupArn: String,
