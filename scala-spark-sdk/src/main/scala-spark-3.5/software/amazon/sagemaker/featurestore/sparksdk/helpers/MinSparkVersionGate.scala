...
/** Spark 3.5 build: allows features requiring Spark 3.5 or older; throws for anything newer. */
object MinSparkVersionGate extends MinSparkVersionGateLike {
  private val builtFor = (3, 5)

  def requireSparkVersion(major: Int, minor: Int): Unit =
    if ((major, minor) > builtFor)
      throw new UnsupportedOperationException(
        s"Feature requires Spark $major.$minor+; this SDK jar was built against Spark 3.5."
      )
}
