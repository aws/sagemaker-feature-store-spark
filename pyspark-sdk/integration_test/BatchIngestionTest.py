...
identity_file_key = (
    "datasets/tabular/fraud_detection/"
    "synthethic_fraud_detection_SA/sampled_identity.csv"
)
identity_data_object = s3_client.get_object(
    Bucket=fraud_detection_bucket_name, Key=identity_file_key
)
csv_data = spark.sparkContext.parallelize(
    identity_data_object["Body"].read().decode("utf-8").split('\r\n')
)
...
feature_store_manager = FeatureStoreManager(
    f"arn:aws:iam::{account_id}:role/feature-store-role"
)
...
identity_df = spark.read.options(
    header='True', inferSchema='True'
).csv(csv_data).limit(20).cache()
identity_df = identity_df.withColumn("EventTime", lit(current_time))

feature_definitions = (
    feature_store_manager.load_feature_definitions_from_schema(identity_df)
)
...
def wait_for_feature_group_creation_complete(feature_group_name):
    ...
    if status != "Created":
        raise RuntimeError(
            f"Failed to create feature group {feature_group_name}"
        )
...
def verify_online_record(ingested_row: Row, record_dict: dict):
    ...
    for key in ingested_row_dict.keys():
        ...
        filterd_record_list = list(
            filter(
                lambda feature_value: feature_value["FeatureName"] == key,
                record_dict
            )
        )
        if ingested_value is not None:
            ...
            tc.assertEqual(
                str(ingested_row_dict[key]),
                filterd_record["ValueAsString"]
            )
        else:
...
def verify_appended_columns(row: Row):
    ...
    tc.assertEqual(
        datetime.fromisoformat(str(row["write_time"])),
...
object_listing = s3.list_objects_v2(
    Bucket=f'spark-test-bucket-{account_id}',
    Prefix=resolved_output_s3_uri.replace(
        f's3://spark-test-bucket-{account_id}/', '', 1
    )
)
